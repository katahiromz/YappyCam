// GaussianBlur_yap.cpp --- PluginFramework Plugin #2
...
static LRESULT DoLoadSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("GaussianBlur_yap"), FALSE);
    if (!hkeyApp)
        return FALSE;

    hkeyApp.QueryDword(TEXT("WindowX"), (DWORD&)s_nWindowX);
    hkeyApp.QueryDword(TEXT("WindowY"), (DWORD&)s_nWindowY);
    hkeyApp.QueryDword(TEXT("BlurX"), (DWORD&)s_nBlurX);
    hkeyApp.QueryDword(TEXT("BlurY"), (DWORD&)s_nBlurY);
...
static LRESULT DoSaveSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("GaussianBlur_yap"), TRUE);
    if (!hkeyApp)
        return FALSE;

    hkeyApp.SetDword(TEXT("WindowX"), s_nWindowX);
    hkeyApp.SetDword(TEXT("WindowY"), s_nWindowY);
    hkeyApp.SetDword(TEXT("BlurX"), s_nBlurX);
    hkeyApp.SetDword(TEXT("BlurY"), s_nBlurY);
...
Plugin_Load(PLUGIN *pi, LPARAM lParam)
{
    ...
    StringCbCopy(pi->plugin_filename, sizeof(pi->plugin_filename), TEXT("GaussianBlur.yap"));
...
static LRESULT Plugin_DoPass(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    cv::Mat& mat = *pmat;
    cv::GaussianBlur(mat, mat, cv::Size(2 * s_nBlurX + 1, 2 * s_nBlurY + 1), 0, 0, cv::BORDER_DEFAULT);
...
static BOOL OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
    ...
    SendDlgItemMessage(hwnd, scr1, UDM_SETPOS, 0, s_nBlurX);
    SendDlgItemMessage(hwnd, scr2, UDM_SETPOS, 0, s_nBlurY);
