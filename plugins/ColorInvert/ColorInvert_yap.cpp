// ColorInvert_yap.cpp --- PluginFramework Plugin #2
...
static LRESULT DoLoadSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("ColorInvert_yap"), FALSE);
    if (!hkeyApp)
        return FALSE;
...
static LRESULT DoSaveSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("ColorInvert_yap"), TRUE);
    if (!hkeyApp)
        return FALSE;
...
Plugin_Load(PLUGIN *pi, LPARAM lParam)
{
    ...
    StringCbCopy(pi->plugin_filename, sizeof(pi->plugin_filename), TEXT("ColorInvert.yap"));
