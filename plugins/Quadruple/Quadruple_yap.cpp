// Quadruple_yap.cpp --- PluginFramework Plugin #2
...
static LRESULT DoResetSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    pi->bEnabled = FALSE;
    pi->dwInfo = PLUGIN_INFO_PASS | PLUGIN_INFO_NOCONFIG;
...
static LRESULT DoLoadSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Quadruple_yap"), FALSE);
    if (!hkeyApp)
        return FALSE;
...
static LRESULT DoSaveSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Quadruple_yap"), TRUE);
    if (!hkeyApp)
        return FALSE;
...
Plugin_Load(PLUGIN *pi, LPARAM lParam)
{
    ...
    StringCbCopy(pi->plugin_filename, sizeof(pi->plugin_filename), TEXT("Quadruple.yap"));
...
static LRESULT Plugin_DoPass(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    cv::Mat& mat = *pmat;
    cv::Mat image;
    cv::resize(mat, image, cv::Size(), 2, 2, cv::INTER_NEAREST);
    mat = image;
