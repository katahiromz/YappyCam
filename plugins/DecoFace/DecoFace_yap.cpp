// DecoFace_yap.cpp --- PluginFramework Plugin #2
...
static std::vector<cv::Rect> s_faces;
static cv::Mat s_deco;
static cv::Mat s_mask;
static INT s_xRatio = 200;
static INT s_yRatio = 200;
...
static LRESULT DoResetSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    pi->bEnabled = FALSE;
    pi->dwInfo = PLUGIN_INFO_PASS | PLUGIN_INFO_USEFACES;
    pi->dwState = PLUGIN_STATE_PASS1;
    pi->p_user_data = NULL;
    pi->l_user_data = 0;
    StringCbCopyW(s_szFileName, sizeof(s_szFileName), L"CatEar.png");
    s_xRatio = 200;
    s_yRatio = 200;
    return 0;
}
...
static LRESULT DoLoadSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("DecoFace_yap"), FALSE);
    if (!hkeyApp)
        return FALSE;

    hkeyApp.QueryDword(TEXT("WindowX"), (DWORD&)s_nWindowX);
    hkeyApp.QueryDword(TEXT("WindowY"), (DWORD&)s_nWindowY);
    hkeyApp.QueryDword(TEXT("xRatio"), (DWORD&)s_xRatio);
    hkeyApp.QueryDword(TEXT("yRatio"), (DWORD&)s_yRatio);
...
static LRESULT DoSaveSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("DecoFace_yap"), TRUE);
    if (!hkeyApp)
        return FALSE;

    hkeyApp.SetDword(TEXT("WindowX"), s_nWindowX);
    hkeyApp.SetDword(TEXT("WindowY"), s_nWindowY);
    hkeyApp.SetDword(TEXT("xRatio"), s_xRatio);
    hkeyApp.SetDword(TEXT("yRatio"), s_yRatio);
...
Plugin_Load(PLUGIN *pi, LPARAM lParam)
{
    ...
    StringCbCopy(pi->plugin_filename, sizeof(pi->plugin_filename), TEXT("DecoFace.yap"));
...
static LRESULT Plugin_DoPass(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    SIZE_T num_faces = (*pi->driver)(pi, DRIVERFUNC_GETFACES, 0, 0);
    if (!num_faces)
        return TRUE;

    s_faces.resize(num_faces);
    (*pi->driver)(pi, DRIVERFUNC_GETFACES, num_faces, (LPARAM)&s_faces[0]);

    if (s_mask.empty())
    {
        for (size_t i = 0; i < s_faces.size(); ++i)
        {
            INT cx = s_faces[i].width * s_xRatio / 100;
            INT cy = s_faces[i].height * s_yRatio / 100;
            INT x = s_faces[i].x + s_faces[i].width / 2;
            INT y = s_faces[i].y + s_faces[i].height / 2;
            cv::Point p0(x - cx / 2, y - cy / 2);
            cv::Point p1(x + cx / 2, y + cy / 2);

            mat = DoPasteFrame(mat, s_deco, p0, p1);
        }
    }
    else
    {
...
static BOOL OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
    ...
    SendDlgItemMessage(hwnd, scr1, UDM_SETRANGE, 0, MAKELONG(400, 50));
    SendDlgItemMessage(hwnd, scr2, UDM_SETRANGE, 0, MAKELONG(400, 50));
    SendDlgItemMessage(hwnd, scr1, UDM_SETPOS, 0, s_xRatio);
    SendDlgItemMessage(hwnd, scr2, UDM_SETPOS, 0, s_yRatio);
...
static void OnEdt1(HWND hwnd)
{
    ...
    INT nValue = GetDlgItemInt(hwnd, edt1, &bTranslated, FALSE);
    if (bTranslated)
        s_xRatio = nValue;
}
...
{
    BOOL bTranslated = FALSE;
    INT nValue = GetDlgItemInt(hwnd, edt2, &bTranslated, FALSE);
    if (bTranslated)
        s_yRatio = nValue;
}
