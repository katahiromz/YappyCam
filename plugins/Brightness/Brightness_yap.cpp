// Brightness_yap.cpp --- PluginFramework Plugin #2
...
static PLUGIN *s_pi;
static INT s_nBrightness;
static INT s_nContrast;
...
static LRESULT DoLoadSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Brightness_yap"), FALSE);
    if (!hkeyApp)
        return FALSE;

    hkeyApp.QueryDword(TEXT("WindowX"), (DWORD&)s_nWindowX);
    hkeyApp.QueryDword(TEXT("WindowY"), (DWORD&)s_nWindowY);
    hkeyApp.QueryDword(TEXT("Brightness"), (DWORD&)s_nBrightness);
    hkeyApp.QueryDword(TEXT("Contrast"), (DWORD&)s_nContrast);
...
static LRESULT DoSaveSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Brightness_yap"), TRUE);
    if (!hkeyApp)
        return FALSE;

    hkeyApp.SetDword(TEXT("WindowX"), s_nWindowX);
    hkeyApp.SetDword(TEXT("WindowY"), s_nWindowY);
    hkeyApp.SetDword(TEXT("Brightness"), s_nBrightness);
    hkeyApp.SetDword(TEXT("Contrast"), s_nContrast);
...
Plugin_Load(PLUGIN *pi, LPARAM lParam)
{
    ...
    StringCbCopy(pi->plugin_filename, sizeof(pi->plugin_filename), TEXT("Brightness.yap"));
...
static LRESULT Plugin_DoPass(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    if (s_nBrightness != 0 || s_nContrast != 100)
    {
        // modify the brightness and contrast values
        double alpha = s_nContrast / 100.0;
        double beta = s_nBrightness;
        mat.convertTo(image1, -1, alpha, beta);
        mat = image1;
    }
...
static BOOL OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
    ...
    SendDlgItemMessage(hwnd, scr1, UDM_SETPOS, 0, MAKELPARAM(s_nBrightness, 0));
    SendDlgItemMessage(hwnd, scr2, UDM_SETPOS, 0, MAKELPARAM(s_nContrast, 0));
