// Quarter_yap.cpp --- PluginFramework Plugin #2
...
static LRESULT DoLoadSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Quarter_yap"), FALSE);
    if (!hkeyApp)
        return FALSE;
...
static LRESULT DoSaveSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Quarter_yap"), TRUE);
    if (!hkeyApp)
        return FALSE;
...
Plugin_Load(PLUGIN *pi, LPARAM lParam)
{
    ...
    StringCbCopy(pi->plugin_filename, sizeof(pi->plugin_filename), TEXT("Quarter.yap"));
...
static LRESULT Plugin_DoPass(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    cv::Mat& mat = *pmat;
    cv::Mat image;
    cv::resize(mat, image, cv::Size(), 0.5, 0.5, cv::INTER_NEAREST);
    mat = image;
