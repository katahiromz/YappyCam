...
static LRESULT DoLoadSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Monochrome_yap"), FALSE);
    if (!hkeyApp)
        return FALSE;
...
static LRESULT DoSaveSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Monochrome_yap"), TRUE);
    if (!hkeyApp)
        return FALSE;
...
Plugin_Load(PLUGIN *pi, LPARAM lParam)
{
    ...
    StringCbCopy(pi->plugin_filename, sizeof(pi->plugin_filename), TEXT("Monochrome.yap"));
...
static LRESULT Plugin_DoPass(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    cv::cvtColor(image, mat, cv::COLOR_GRAY2BGR);
