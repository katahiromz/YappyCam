// Resizing_yap.cpp --- PluginFramework Plugin #2
...
static PLUGIN *s_pi;
static INT s_nResizingX;
static INT s_nResizingY;
...
static LRESULT DoLoadSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Resizing_yap"), FALSE);
    if (!hkeyApp)
        return FALSE;

    hkeyApp.QueryDword(TEXT("WindowX"), (DWORD&)s_nWindowX);
    hkeyApp.QueryDword(TEXT("WindowY"), (DWORD&)s_nWindowY);
    hkeyApp.QueryDword(TEXT("ResizingX"), (DWORD&)s_nResizingX);
    hkeyApp.QueryDword(TEXT("ResizingY"), (DWORD&)s_nResizingY);
...
static LRESULT DoSaveSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Resizing_yap"), TRUE);
    if (!hkeyApp)
        return FALSE;

    hkeyApp.SetDword(TEXT("WindowX"), s_nWindowX);
    hkeyApp.SetDword(TEXT("WindowY"), s_nWindowY);
    hkeyApp.SetDword(TEXT("ResizingX"), s_nResizingX);
    hkeyApp.SetDword(TEXT("ResizingY"), s_nResizingY);
...
Plugin_Load(PLUGIN *pi, LPARAM lParam)
{
    ...
    StringCbCopy(pi->plugin_filename, sizeof(pi->plugin_filename), TEXT("Resizing.yap"));
...
static LRESULT Plugin_DoPass(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    cv::Mat& mat = *pmat;
    cv::resize(mat, mat, cv::Size(s_nResizingX, s_nResizingY), 0, 0, cv::INTER_LINEAR);
...
static BOOL OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
    ...
    SendDlgItemMessage(hwnd, scr1, UDM_SETPOS, 0, s_nResizingX);
    SendDlgItemMessage(hwnd, scr2, UDM_SETPOS, 0, s_nResizingY);
