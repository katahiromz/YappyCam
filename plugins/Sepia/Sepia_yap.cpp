// Sepia_yap.cpp --- PluginFramework Plugin #2
...
static LRESULT DoLoadSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Sepia_yap"), FALSE);
    if (!hkeyApp)
        return FALSE;

    hkeyApp.QueryDword(TEXT("WindowX"), (DWORD&)s_nWindowX);
    hkeyApp.QueryDword(TEXT("WindowY"), (DWORD&)s_nWindowY);
    hkeyApp.QueryDword(TEXT("Darkness"), (DWORD&)s_nDarkness);
...
static LRESULT DoSaveSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("Sepia_yap"), TRUE);
    if (!hkeyApp)
        return FALSE;

    hkeyApp.SetDword(TEXT("WindowX"), s_nWindowX);
    hkeyApp.SetDword(TEXT("WindowY"), s_nWindowY);
    hkeyApp.SetDword(TEXT("Darkness"), s_nDarkness);
...
Plugin_Load(PLUGIN *pi, LPARAM lParam)
{
    ...
    StringCbCopy(pi->plugin_filename, sizeof(pi->plugin_filename), TEXT("Sepia.yap"));
...
#define PIXVAL(mat, x, y) (mat).at<BYTE>(y, x)
#define PIXVALR(mat, x, y) (mat).at<cv::Vec3b>(y, x)[2]
#define PIXVALG(mat, x, y) (mat).at<cv::Vec3b>(y, x)[1]
#define PIXVALB(mat, x, y) (mat).at<cv::Vec3b>(y, x)[0]
    ...
    for (int y = 0; y < gray.rows; ++y)
    {
        ...
        {
            int g = PIXVAL(gray, x, y) - s_nDarkness;
            PIXVAL(gray, x, y) = BYTE((g > 0) ? g : 0);
        }
    }
    ...
    for (int y = 0; y < mat.rows; ++y)
    {
        ...
        {
            int r = PIXVALR(mat, x, y) + 30;
            int b = PIXVALB(mat, x, y) - 30;
            PIXVALR(mat, x, y) = BYTE((r < 256) ? r : 255);
            PIXVALB(mat, x, y) = BYTE((b > 0) ? b : 0);
        }
    }
...
static BOOL OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam)
{
    ...
    SendDlgItemMessage(hwnd, scr1, UDM_SETPOS, 0, s_nDarkness);
