// FaceDetection_yap.cpp --- PluginFramework Plugin #2
...
static LRESULT DoResetSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    pi->bEnabled = FALSE;
    pi->dwInfo = PLUGIN_INFO_PASS | PLUGIN_INFO_NOCONFIG | PLUGIN_INFO_USEFACES;
...
static LRESULT DoLoadSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("FaceDetection_yap"), FALSE);
    if (!hkeyApp)
        return FALSE;
...
static LRESULT DoSaveSettings(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    MRegKey hkeyApp(hkeyCompany, TEXT("FaceDetection_yap"), TRUE);
    if (!hkeyApp)
        return FALSE;
...
Plugin_Load(PLUGIN *pi, LPARAM lParam)
{
    ...
    StringCbCopy(pi->plugin_filename, sizeof(pi->plugin_filename), TEXT("FaceDetection.yap"));
...
static LRESULT Plugin_DoPass(PLUGIN *pi, WPARAM wParam, LPARAM lParam)
{
    ...
    {
        cv::rectangle(mat,
                      cv::Point(s_faces[i].x, s_faces[i].y),
                      cv::Point(s_faces[i].x + s_faces[i].width, s_faces[i].y + s_faces[i].height),
                      cv::Scalar(0, 0, 255),
                      3);
    }
